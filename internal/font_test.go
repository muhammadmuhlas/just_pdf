...
func TestFont_GetSetFont(t *testing.T) {
	cases := []struct {
		...
		{
			"PdfJustPdf.Arial, PdfJustPdf.Normal, 16",
			consts.Arial,
			consts.Normal,
			16.0,
			func() *mocks.Pdf {
		...
		{
			"PdfJustPdf.Helvetica, PdfJustPdf.Bold, 13",
			consts.Helvetica,
			consts.Bold,
			13,
			func() *mocks.Pdf {
		...
		{
			"PdfJustPdf.Symbol, PdfJustPdf.Italic, 10",
			consts.Symbol,
			consts.Italic,
			10,
			func() *mocks.Pdf {
		...
		{
			"PdfJustPdf.ZapBats, PdfJustPdf.BoldItalic, 5",
			consts.ZapBats,
			consts.BoldItalic,
			5,
			func() *mocks.Pdf {
		...
		{
			"PdfJustPdf.Courier, PdfJustPdf.Normal, 12",
			consts.Courier,
			consts.Normal,
			12,
			func() *mocks.Pdf {
