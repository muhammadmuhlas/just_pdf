...
type TableList struct {
	...
	// the headers
	HeaderProp Font
	// HeaderColor for coloerd header
	HeaderColor *color.Color
	// ContentFontColor is color of text on content
	ContentFontColor *color.Color
	// HeaderFontColor is color of text on header
	HeaderFontColor *color.Color
	...
	Align       consts.Align
	// CustomAlign override Align with customized each column
	CustomAlign []consts.Align
	...
	// AlternatedOddBackground define the background color from odd rows
	// i.e rows with index (1, 3, 5, ..., N) will have background colorized
	AlternatedOddBackground *color.Color
...
func (s *TableList) MakeValid() {
	...
	if s.HeaderColor == nil {
		defaultColor := color.NewBlack()
		s.HeaderColor = &defaultColor
	}
}
