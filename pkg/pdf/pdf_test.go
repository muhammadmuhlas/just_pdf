...
func TestNewPdf(t *testing.T) {
	cases := []struct {
		...
		assert      func(t *testing.T, m pdf.JustPdf)
	}{
		{
			"When portrait and A4",
			consts.Portrait,
			consts.A4,
			func(t *testing.T, m pdf.JustPdf) {
				assert.NotNil(t, m)
				assert.Equal(t, fmt.Sprintf("%T", m), "*pdf.PdfJustPdf")
	...
	for _, c := range cases {
		// Act
		m := pdf.NewJustPdf(c.orientation, c.pageSize)
...
func TestPdfJustPdf_SetGetDebugMode(t *testing.T) {
	// Arrange
...
func TestPdfJustPdf_Signature(t *testing.T) {
	...
	for _, c := range cases {
		...
		pdf := basePdfTest(10, 10, 10, 10)
		math := baseMathTest()
		tableList := baseTableList()
		m := newJustPdfTest(pdf, math, nil, nil, signature, nil, nil, tableList)
...
func TestPdfJustPdf_Text(t *testing.T) {
	cases := []struct {
		name   string
	...
	for _, c := range cases {
		...
		math := baseMathTest()
		tableList := baseTableList()
		m := newJustPdfTest(pdf, math, nil, text, nil, nil, nil, tableList)
...
func TestPdfJustPdf_FileImage(t *testing.T) {
	cases := []struct {
		name   string
	...
	for _, c := range cases {
		...
		m := newJustPdfTest(pdf, math, nil, nil, nil, image, nil, tableList)
...
func TestPdfJustPdf_Base64Image(t *testing.T) {
	cases := []struct {
		name   string
		image  func() *mocks.Image
		assert func(t *testing.T, image *mocks.Image)
...
func TestPdfJustPdf_QrCode(t *testing.T) {
	cases := []struct {
		name   string
	...
	for _, c := range cases {
		...
		m := newJustPdfTest(pdf, math, nil, nil, nil, nil, code, tableList)
...
func TestPdfJustPdf_Barcode(t *testing.T) {
	cases := []struct {
		name   string
		code   func() *mocks.Code
		assert func(t *testing.T, image *mocks.Code)
...
func TestPdfJustPdf_Row(t *testing.T) {
	cases := []struct {
		name                 string
		act                  func(m pdf.JustPdf, calledTimes *int)
		assertRowCalledTimes func(t *testing.T, calledTimes int)
		assertPdfCalls       func(t *testing.T, pdf *mocks.Pdf)
	}{
		{
			"One row",
			func(m pdf.JustPdf, calledTimes *int) {
				m.Row(30, func() {
					*calledTimes++
				})
			},
	...
	for _, c := range cases {
		...
		m := newJustPdfTest(pdf, math, nil, nil, nil, nil, nil, tableList)
		calledTimes := 0
...
func TestPdfJustPdf_Line(t *testing.T) {
	cases := []struct {
		name   string
		pdf    func() *mocks.Pdf
...
func TestPdfJustPdf_ColSpace(t *testing.T) {
	cases := []struct {
		name   string
		act    func(m pdf.JustPdf)
		assert func(t *testing.T, pdf *mocks.Pdf)
...
func TestPdfJustPdf_ColSpaces(t *testing.T) {
	cases := []struct {
		name   string
		act    func(m pdf.JustPdf)
		assert func(t *testing.T, pdf *mocks.Pdf)
...
func TestPdfJustPdf_Output(t *testing.T) {
	cases := []struct {
		name              string
		pdf               func() *mocks.Pdf
		hasFooter         bool
...
func TestPdfJustPdf_OutputFileAndClose(t *testing.T) {
	cases := []struct {
		name           string
		pdf            func() *mocks.Pdf
		assertPdfCalls func(t *testing.T, pdf *mocks.Pdf)
...
func newJustPdfTest(fpdf *mocks.Pdf, math *mocks.Math, font *mocks.Font, text *mocks.Text,
	signature *mocks.Signature, image *mocks.Image, code *mocks.Code, tableList *mocks.TableList) pdf.JustPdf {
	m := &pdf.PdfJustPdf{
		Pdf:             fpdf,
		Math:            math,
		Font:            font,
...
func basePdfTest(left, top, right, bottom float64) *mocks.Pdf {
	pdf := &mocks.Pdf{}
	pdf.On("GetPageSize").Return(100.0, 100.0)
	pdf.On("GetMargins").Return(left, top, right, bottom)
...
func TestPdfJustPdf_RegisterFooter(t *testing.T) {
	cases := []struct {
		name      string
		act       func(m pdf.JustPdf)
		hasFooter bool
	...
	for _, c := range cases {
		...
		m := newJustPdfTest(pdf, math, font, text, nil, nil, nil, tableList)
		...
		pdf.On("PageCount").Return(0)
		//math.On("PageCount").Return(0)
		//font.On("PageCount").Return(0)
		//tableList.On("PageCount").Return(0)
		// Act
		c.act(m)
...
func TestPdfJustPdf_RegisterHeader(t *testing.T) {
	cases := []struct {
		name       string
		act        func(m pdf.JustPdf)
		hasClosure bool
...
func TestPdfJustPdf_GetCurrentPage(t *testing.T) {
	cases := []struct {
		name   string
		act    func(m pdf.JustPdf)
...
func TestPdfJustPdf_GetCurrentPage_WhenCreateOffsetIsZero(t *testing.T) {
	// Arrange
	pdf := basePdfTest(10, 10, 10, 10)
	math := baseMathTest()
	text := baseTextTest()
...
func TestPdfJustPdf_GetCurrentPage_WhenIsNotZero(t *testing.T) {
	// Arrange
	pdf := basePdfTest(10, 10, 10, 10)
	math := baseMathTest()
	text := baseTextTest()
...
func TestPdfJustPdf_SetPageMargins(t *testing.T) {
	cases := []struct {
		name   string
		act    func(m pdf.JustPdf)
		assert func(t *testing.T, left, top, right, bottom float64)
	}{
		{
			"Set page margins should override default",
			func(m pdf.JustPdf) {
				m.SetPageMargins(12.3, 19.3, 0.0, 20)
			},
			func(t *testing.T, left, top, right, bottom float64) {
	...
	for _, c := range cases {
		// Arrange
		pdf := basePdfTest(12.3, 19.3, 0, 10)

		pdf.On("SetY", 19.3).Return(nil)
		pdf.On("SetTopMargin", 19.3).Return(nil)
		pdf.On("SetLeftMargin", 12.3).Return(nil)
		pdf.On("SetRightMargin", 0.0).Return(nil)
		pdf.On("SetAutoPageBreak", true, 20.0).Return(nil)
		m := newJustPdfTest(pdf, nil, nil, nil, nil, nil, nil, nil)
		...
		c.assert(t, left, top, right, bottom)
	}
}

func TestPdfJustPdf_SetBackgroundColor(t *testing.T) {
	// Arrange
	pdf := basePdfTest(12.3, 19.3, 0, 10)
	m := newJustPdfTest(pdf, nil, nil, nil, nil, nil, nil, nil)
